...
def dashboard():
    ...
    registrations = db.execute("""
        ...
        FROM registrations r JOIN opportunities o ON o.id=r.opportunity_id
        WHERE r.user_id=? ORDER BY o.event_date
...
def opportunities():
    ...
    return render_template("opportunities.html", opportunities=rows, search=search, category=category)
...
def opportunity_detail(opportunity_id):
    ...
    return render_template("opportunity_detail.html", opportunity=opportunity, already=already)
...
def register_opportunity(opportunity_id):
    ...
    try:
        db.execute("INSERT INTO registrations(user_id,opportunity_id) VALUES(?,?)",
                   (session["user_id"],opportunity_id))
        db.commit()
        flash("Registration submitted. Wait for organizer approval.", "success")
    except sqlite3.IntegrityError:
...
def organizer_dashboard():
    ...
    own=db.execute("""
        SELECT o.*,
        (SELECT COUNT(*) FROM registrations r WHERE r.opportunity_id=o.id) AS registration_count
        FROM opportunities o WHERE o.organizer_id=? ORDER BY o.event_date
    """,(session["user_id"],)).fetchall()
...
def create_opportunity():
    if request.method=="POST":
        ...
        if not all([title,description,category,location,event_date,capacity]):
            flash("All fields are required.","danger")
        ...
        try:
            ...
            return render_template("opportunity_form.html", opportunity=None)
        get_db().execute("""
            INSERT INTO opportunities(title,description,category,location,event_date,capacity,organizer_id)
            VALUES(?,?,?,?,?,?,?)
        """,(title,description,category,location,event_date,capacity,session["user_id"]))
        get_db().commit()
        flash("Opportunity created.","success")
        return redirect(url_for("organizer_dashboard"))
    return render_template("opportunity_form.html", opportunity=None)

@app.route("/organizer/opportunity/<int:opportunity_id>/edit", methods=["GET","POST"])
@login_required
@role_required("organizer")
def edit_opportunity(opportunity_id):
    db=get_db()
    opportunity=db.execute("SELECT * FROM opportunities WHERE id=? AND organizer_id=?",
                           (opportunity_id,session["user_id"])).fetchone()
    if not opportunity:
    ...
    if request.method=="POST":
        ...
        try:
            capacity=int(capacity)
            datetime.strptime(event_date,"%Y-%m-%d")
            if capacity<1 or not all([title,description,category,location]): raise ValueError
        except ValueError:
            flash("Please enter valid values.","danger")
            return render_template("opportunity_form.html", opportunity=opportunity)
        db.execute("""
            UPDATE opportunities SET title=?,description=?,category=?,location=?,event_date=?,capacity=?
            WHERE id=? AND organizer_id=?
        """,(title,description,category,location,event_date,capacity,opportunity_id,session["user_id"]))
        db.commit()
        flash("Opportunity updated.","success")
        return redirect(url_for("organizer_dashboard"))
    return render_template("opportunity_form.html", opportunity=opportunity)

@app.route("/organizer/opportunity/<int:opportunity_id>/delete", methods=["POST"])
@login_required
@role_required("organizer")
def delete_opportunity(opportunity_id):
    db=get_db()
    db.execute("DELETE FROM opportunities WHERE id=? AND organizer_id=?",
...
def registrations(opportunity_id):
    ...
    if not opportunity:
        ...
        return redirect(url_for("organizer_dashboard"))
    rows=db.execute("""
        SELECT r.*,u.name,u.email FROM registrations r JOIN users u ON u.id=r.user_id
        WHERE r.opportunity_id=? ORDER BY r.created_at
    """,(opportunity_id,)).fetchall()
    return render_template("registrations.html", opportunity=opportunity, registrations=rows)

@app.route("/organizer/registration/<int:registration_id>/<action>", methods=["POST"])
@login_required
@role_required("organizer")
def update_registration(registration_id, action):
    ...
    db=get_db()
    row=db.execute("""
        SELECT r.id FROM registrations r JOIN opportunities o ON o.id=r.opportunity_id
    ...
    if not row:
        ...
        return redirect(url_for("organizer_dashboard"))
    if action=="attended":
        db.execute("UPDATE registrations SET attendance=1,status='approved' WHERE id=?",(registration_id,))
    else:
        db.execute("UPDATE registrations SET status=? WHERE id=?",(action,registration_id))
    db.commit()
